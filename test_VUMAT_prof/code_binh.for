...
C   STATE(*,2) = temperature   
C    
C   We are not using the temperature provided by Abaqus: tempOld and tempNew
C   but we are using the temperature from the previous increment
C
...
C   Check if this is 2D or 3D. 
C   If 2D then ndir = 3, nshr = 1 
C   If 3D then ndir = 3, nshr = 3
	      if ( nshr .gt. 1 ) then
